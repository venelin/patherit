...
stepContTimenxNorm <- function(N=Inf, nu=0, mu,
  ...
  # increment time spent in current viral genotype for all active
  for(gi in 1:nGtps) {
    taugi <- gen[active==1&gene==gi, paste0('taug',gi), with=FALSE]
    gen[active==1&gene==gi, paste0('taug',gi):=taugi+1]
  }
